...
Для данной плёнки алгоритм приводит к ветви: регулярная тахикардия → зубец P виден. Ретроградные зубцы P на плёнке присутствуют — как псевдо-S в aVF и положительная зазубрина в aVL в терминальной части QRS. Далее оценивается RP-интервал: при классической АВУРТ slow-fast он короткий (RP ≤ 90 мс, короче PR), потому что ретроградная предсердная активация почти совпадает с желудочковой. Это и даёт диагноз **типичной АВУРТ**.

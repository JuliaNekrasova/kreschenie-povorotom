...
<figure class="ekg">
  ...
  <figcaption>Рис. 4. Дифференциальный диагноз узкокомплексных тахикардий (QRS ≤ 120 мс). Сначала определяется регулярность ритма, затем видимость зубцов P и соотношение предсердной и желудочковой активности. Источник: ESC Guidelines 2019.</figcaption>
</figure>

Пройдём алгоритм по нашей плёнке:

1. **Тахикардия регулярная?** — Да. Интервалы R–R одинаковы. Нерегулярные варианты (ФП, ТП с переменным проведением, полифокусная ПТ) уходят в правую ветвь и сразу исключаются.
2. **Зубец P виден?** — Да. Ретроградные зубцы P присутствуют: псевдо-S в aVF и положительная зазубрина в aVL в терминальной части QRS.
3. **Предсердный ритм чаще желудочкового?** — Нет. Соотношение P:QRS = 1:1. Если бы предсердный ритм был чаще (больше P, чем QRS), мы ушли бы в ветвь **трепетания предсердий или фокусной ПТ с АВ-блокадой** — там предсердия работают быстрее, чем АВ-узел пропускает. У нас не так.
4. **Желудочковый ритм чаще предсердного?** — Нет. Опять 1:1. Если бы желудочков было больше — ветвь высокой септальной ЖТ, фокусной АВ-тахикардии.
5. **Оценка RP-интервала.** При проведении 1:1 и видимых P алгоритм приводит сюда. RP-интервал короткий (RP ≤ 90 мс, RP < PR) — ретроградная предсердная активация почти совпадает с желудочковой. Это ветвь **типичной АВУРТ**, фокусной ПТ и АВРТ (редко).

Окончательное отнесение к АВУРТ, а не к АВРТ или фокусной ПТ: нет дельта-волны на базовых ЭКГ 2 и 3 (манифестный WPW исключён), пароксизм купирован вагусным стимулом (фокусная ПТ обычно нечувствительна к вагусным маневрам), молодая женщина с рецидивирующими пароксизмами (АВУРТ составляет 50–60 % всех пароксизмальных НЖТ).

## Алгоритм неотложной терапии

<figure class="ekg">
  <img src="images/esc_therapy.png" alt="Неотложная терапия тахикардий с узкими QRS по ESC 2019">
  <figcaption>Рис. 5. Алгоритм неотложной терапии узкокомплексной тахикардии при неясном диагнозе. Первое разветвление — гемодинамика. Источник: ESC Guidelines 2019 (с изменениями).</figcaption>
</figure>
...
При неэффективности АТФ/аденозина и сохраняющейся стабильной гемодинамике — **верапамил** 2,5–5 мг внутривенно медленно (ESC, класс IIa B) или **бета-блокатор** (ESC, класс IIa C). Верапамил противопоказан при желудочковой тахикардии, сниженной фракции выброса, гипотензии и на фоне приёма бета-блокаторов.

### Российские клинические рекомендации

Клинические рекомендации «Наджелудочковые тахикардии» (утв. Минздрав России) и Стандарт скорой медицинской помощи при тахикардиях и тахиаритмиях (Приказ № 464н) повторяют ту же иерархию:

- **Нестабильная гемодинамика** — синхронизированная ЭИТ.
- **Стабильная гемодинамика** — вагусные приёмы → трифосаденин (АТФ) → верапамил или бета-блокаторы.
- Постоянный мониторинг ЭКГ при введении антиаритмических препаратов.

Различие в деталях: ESC использует аденозин, российские рекомендации — трифосаденин (АТФ). Принцип тот же: **стабильна ли пациентка? Если нет — ЭИТ. Если да — вагусные маневры, затем АВ-узловый блокатор**.
...
## Изменения T после конверсии: Т-память или ишемия?

Инверсия зубца T в II, III, aVF сразу после остановки тахикардии — это может быть либо **постконверсионная Т-память** (cardiac memory), либо **транзиторная ишемия**, связанная с высокой частотой. Объяснить изменения однозначно по одной ЭКГ нельзя: у обеих гипотез одинаковые ЭКГ-признаки, и у обеих может быть отрицательный тропонин, если нет некроза.

Механизм Т-памяти: во время тахикардии желудочек активируется аномальным путём (в данном случае — с аберрацией по типу блокады правой ножки). Реполяризация подстраивается под изменённую деполяризацию. Когда нормальный ритм восстанавливается, деполяризация мгновенно возвращается к норме, а реполяризация — нет. Миокард «помнит» аномальный паттерн.

Аргументы за Т-память

- Пароксизм длился менее двух минут — короткая аномальная активация, после которой Т-память разрешается за минуты или десятки минут.
- Через 30 минут третья плёнка показала полное разрешение инверсии T.
- После конверсии пациентка была бессимптомна: нет боли, одышки, гемодинамической нестабильности.

Аргументы за ишемию, в том числе частотно-зависимую

- Частота 192 в минуту резко сокращает диастолу и время коронарного перфузионного наполнения. Даже при незатронутых артериях это может вызывать субэндокардиальную ишемию, особенно на фоне гипотензии (90/55) и рвоты.
- Ишемия и Т-память могут сосуществовать: обе дают инверсию T, обе могут обратиться за 30 минут после снижения частоты и нормализации перфузии.
- Транзиторная ишемия может не повреждать миокард — тропонин при ней часто остаётся отрицательным. Поэтому тропонин не различает Т-память и бесклеточнонекротическую ишемию; различить их можно по динамике, клинике и, при необходимости, нагрузочным исследованиям.

Что это значит на линии?

- Однозначно заявлять «это не ишемия» — нельзя. Точнее: «наиболее вероятная причина — Т-память на фоне короткой тахикардии с аберрацией, но транзиторная ишемия исключена не полностью».
- Пациентка стабильна, третья плёнка через 30 минут нормализовалась — это в пользу обратимого, не требующего реперфузии, механизма.
- Передать приёмному покою: ЭКГ с инверсией T после остановки НЖТ 192/мин, разрешение через 30 минут; клинически без ишемии. Решение о тропонинах и консультации кардиолога остаётся за приёмным покоем.
...
# Значение рутинной ЭКГ

ЭКГ в этом вызове было подключено рутинно — повод не предполагал аритмию. Пульс на пульсоксиметре составлял 95 в минуту, аускультативно ничего необычного.

Если бы ЭКГ не записали, пациентка осталась бы без диагноза — как оставалась предыдущие 10 лет. Боль в груди с 14 лет, непереносимость нагрузки, и ни одного обращения к кардиологу. Случайно зафиксированная двухминутная пробежка, пойманная между подключением и отключением электродов, дала основание для направления на ЭФИ и возможной абляции.

...
Исходное положение авторов: рекомендации 2025 года не содержат принципиально новых средств, и рост выживаемости в сильных системах достигнут не новыми препаратами, а качеством исполнения — сокращением времени до вызова бригады, долей случаев с СЛР от очевидцев, ранним доступом к автоматическому дефибриллятору и слаженностью работы бригады. Под системой здесь имеется в виду организация помощи на территории: диспетчеризация, доля СЛР очевидцами, доступность автоматических дефибрилляторов, слаженность бригады и наличие измерения качества компрессий с обратной связью. «Сильная система» в этом смысле — устойчивое исполнение этих звеньев, а не набор препаратов. Основное содержание выпуска относится к тем точкам, где алгоритм заканчивается: фибрилляция желудочков, сохраняющаяся после трёх разрядов, адреналина и амиодарона, — состояние, для которого рекомендации 2025 года прямых указаний не дают.
...
- Когда качество компрессий начали измерять объективно, оказалось, что высокого качества не обеспечивала практически ни одна служба: без измерения бригада не видит глубину, частоту и полноту отпускания грудины и не может их поправить. Улучшение наступало только после введения измерения и обратной связи.
...
<figure>
  ...
  <figcaption>Трактовка EtCO₂ в изложении выпуска. Значения — для взрослого при продолжающихся компрессиях. Порог «ниже 10» читается не в первую минуту, а после примерно 20 минут качественной СЛР.</figcaption>
</figure>
...
- Значение искажают: утечка при масочной вентиляции и отсутствие герметичного контура, исходная задержка углекислого газа у пациента, длительность остановки, качество компрессий, массивная тромбоэмболия, введение гидрокарбоната натрия (даёт кратковременный подъём EtCO₂ за счёт выделившегося углекислого газа, а не за счёт кровотока).
...
- После постановки надгортанного устройства или эндотрахеальной трубки рекомендованный режим прежний: один вдох каждые 6 секунд, продолжительность вдоха около секунды, объём — минимально достаточный для видимого подъёма грудной клетки.
...
<figure>
  ...
  <figcaption>Речь не о восстановлении кровообращения. Разряд мог перевести фибрилляцию в другой ритм, затем фибрилляция вернулась в первые 30 секунд. Следующая плановая оценка ритма — только через две минуты: до полутора минут бригада продолжает компрессии уже снова фибриллирующего сердца и об этом не знает.</figcaption>
</figure>

- Большинство рецидивов фибрилляции возникает в течение примерно 30 секунд после разряда, а не через две минуты. Это повторное появление фибрилляции у пациента, у которого кровообращение не восстановилось: монитор после разряда мог показать другой ритм, затем снова фибрилляцию.
- Отсюда довод: двухминутный цикл — удобный учебный приём, а не физиологическая закономерность. При жёстком следовании ему ритм между разрядом и плановой проверкой не читается: если фибрилляция вернулась на двадцатой секунде, до следующей оценки остаётся около полутора минут, в течение которых компрессии идут, а повторный разряд не наносится.
...
- По сосудистому доступу приводится иерархия: внутривенный предпочтителен, внутрикостный — запасной. Причина не в том, что внутрикостный не работает, а в менее предсказуемом поступлении препарата и в возможности незаметной неудачи при установке. Незаметная неудача означает, что игла выглядит стоящей в кости, а раствор уходит в мягкие ткани или не достигает костномозгового канала. Аспирация костного мозга этот сбой не исключает: при правильном положении содержимое часто не набирается, а при смещении может появиться кровь или жировая капля. Отёк конечности во время компрессий развивается поздно и под одеждой не виден. Доплеровское исследование во время промывания показывает поток в канале кости и отличает работающий доступ от неработающего.
- Отдельное мнение экспертов: при отсутствии внутривенного доступа они предпочитают внутримышечный адреналин внутрикостному, если нет возможности быстро подтвердить положение иглы ультразвуком описанным способом.
...
- СЛР с приподнятым головным концом оценивается скептически. В обсуждаемых работах это не просто приподнять голову на носилках. Методика собрана из трёх частей сразу: контролируемый подъём головы и грудной клетки, механические компрессии и устройство порогового сопротивления вдоху (impedance threshold device) — клапан на контуре вентиляции, который во время отпускания грудины не пускает воздух в лёгкие пассивно; за счёт этого внутригрудное давление становится более отрицательным и венозный возврат увеличивается. Отдельно приподнять голову, продолжая ручной массаж, — не та методика. Данные на животных убедительны, данные на людях — малочисленные и противоречивые; воспроизвести результаты центра, сообщившего об успехе, другим службам не удалось. Отмечена и техническая трудность: механическое устройство смещается на наклонной поверхности.
...
В выпуске сравниваются три способа, а не «воздуховод» в бытовом смысле. Ротоглоточный воздуховод (типа Гведела) здесь не имеется в виду: он удерживает корень языка при вентиляции мешком и маской и самостоятельным способом обеспечения проходимости при остановке не считается.

<div class="sposoby">

| Способ | Что это | Место в изложении выпуска |
|---|---|---|
| Мешок и маска | Вентиляция через лицевую маску, без устройства в гортани | Допустима, если герметичность и объём удерживаются; утечка искажает EtCO₂ |
| Надгортанное устройство | Ларингеальная маска, i-gel, ларингеальная трубка — садится *над* голосовой щелью | В службах, где интубация выполняется редко, названо обычно лучшим выбором |
| Эндотрахеальная трубка | Трубка *через* голосовую щель | В приёмном отделении при опытном исполнителе предпочтительна; компрессии ради неё не прерываются |
...
- Если бригада доставила пациента с работающим надгортанным устройством и вентиляция адекватна, разумно оставить его на время реанимации и заменить после восстановления кровообращения.
...
18. Какие случаи из практики оправдывали введение кальция, гидрокарбоната натрия или магния, и как выглядело обоснование в карте вызова?
